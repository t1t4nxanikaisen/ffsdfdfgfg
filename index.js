...
// Socket.IO client - only import in browser environment
let socket;
...
function CommentsSection({ animeId, episodeId }) {
  ...
  useEffect(() => {
    ...
    // Join episode room
    socket.emit('join_episode', { animeId, episodeId });
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      const result = await commentsUtils.deleteComment(commentId);
      if (result.success) {
...
function ChatSection({ animeId, episodeId }) {
  ...
  useEffect(() => {
    ...
    // Join chat room
    socket.emit('join_chat', { animeId, episodeId });
    ...
    // Load chat history
    socket.emit('get_chat_history', { animeId, episodeId }, (history) => {
      setMessages(history || []);
    });
              ...
              <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-700">
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 bg-[#2d2d2d] border border-gray-600 rounded-lg px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 text-sm"
                    disabled={sending}
                  />
                  <button
                    type="submit"
...
// Main Watch Component (rest of your existing code remains the same)
      ...
      <div className="w-full max-w-[1920px] mx-auto pt-16 pb-6 w-full max-[1200px]:pt-12">
        <div className="grid grid-cols-[minmax(0,70%),minmax(0,30%)] gap-6 w-full h-full max-[1200px]:flex max-[1200px]:flex-col">
          ...
          <div className="flex flex-col w-full gap-6">
            ...
            {/* Rest of your existing Watch component JSX remains exactly the same */}
            {/* ... (all the seasons, episodes, anime info sections) ... */}
          </div>
